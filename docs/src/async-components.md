...
But if this lens getter moved into a async context the value can become `None` during the async tasks execution.
...
# extern crate natrix;
# use natrix::prelude::*;
fn use_string<S: State>(subs: impl Getter<S, u8>) -> impl Element<S> {
