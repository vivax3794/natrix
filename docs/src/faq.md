...
Not really, the trait the `#[derive(State)]` macro implements is `#[doc(hidden)]` for a reason, and we might make semver-breaking changes to it in non-breaking releases.
